...
//exit;
 	...
 	$sql = " SELECT vendor_id, name FROM vendors WHERE login = '$login' ";
 	  
	if(!mysqli_query( $conn, $sql)) { 
		$error = 'Database Error (ff). ' . mysql_error();
		include 'error.php';
		exit;
	}
	...
	if($vresult==FALSE) {
		...
		$error = 'Database Error (fg): ' . mysqli_error();
		include 'error.php';
		exit;
	}
	
	$vnumrows = mysqli_num_rows($vresult);
	$vnumcols = mysqli_num_fields($vresult);	

	if($vnumrows == 1) {
			
		$data = mysqli_fetch_array($vresult);
		
		$vendor_id = $data[0];
		$vendor_name = $data[1];
	}		 
	else {
		header('Location: login.php?error'); //bad login
		exit;
	}
...
//	}	
	...
	$location = 'Location: cms.php?login=' . $login;
...
//	if(strlen($date)!=10 || substr($date,4,1)!="-" || substr($date,7,1)!="-" ) {
	...
	//}	
 	...
 	$sql = " INSERT INTO menuitems ( name, description, price, category, vendor_id )  VALUES (  '$name', '$description', '$price', '$category', $vendor_id ) "; 
...
//	$sql= "SELECT DATE_FORMAT(date,'%W the %D of %M, %Y'),DATE_FORMAT(start_time,'%h:%i %p'),
	...
	$sql= "SELECT category, name, description,  price, menuitem_id, vendor_id FROM menuitems  WHERE vendor_id = $vendor_id ORDER BY category DESC, name DESC";
